...
<div class="container">
    <?php
    $conn = dbConnect();
    echo '<h3>Eventos a los que esta apuntado</h3>';
    echo '<hr>';
	...
	?>
        <script>document.getElementById("buttonEventosApuntado").className += " active";</script>
		...
		$sql = "SELECT * FROM evento, usuario_asiste_evento AS asiste "
			. " WHERE evento.id=asiste.idEvento AND asiste.idUsuario=$idUsuario AND evento.estado=1 AND evento.fecha>='$hoy' "
